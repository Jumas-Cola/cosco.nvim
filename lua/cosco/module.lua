...
M.has_unactionable_lines = function(config)
    ...
    if (config.cosco_ignore_comment_lines == 1) then
        ...
        if string.match(synName:lower(), "comment") then return true end
    end

    -- Ignores empty lines or lines ending with opening ([{
    if (M.strip(vim.b.current_line) == '' or
        string.match(vim.b.current_line_last_char, '[{[(]')) then return true end

    -- Ignores lines if the next one starts with a "{"
    if vim.b.next_line_first_char == '{' then return true end
...
M.ignore_current_filetype = function(config)
    ...
    if config.cosco_filetype_whitelist then
        for _, i in ipairs(config.cosco_filetype_whitelist) do
            if vim.tbl_contains(filetypes, i) then return false end
        end
        return true
    elseif config.cosco_filetype_blacklist then
        for _, i in ipairs(config.cosco_filetype_blacklist) do
            if vim.tbl_contains(filetypes, i) then return true end
        end
        return false
    end
...
M.comma_or_semi_colon = function(config)
    ...
    if M.ignore_current_filetype(config) then return end
    ...
    if M.has_unactionable_lines(config) then return end
